...
golazo <- function(S,L,U,tol=1e-7,verbose=TRUE){
  ...
  while(dualgap > tol){
    ...
    visit <- 1:d # a priori we visit all the nodes
    for (j in visit){
      if (length(eqs[[j]])==0){
        # in our canonical applications this is what we end up doing
        b <- c(S[j,-j]+L[j,-j],-(S[j,-j]+U[j,-j]))
        y <- quadprog::solve.QP(Dmat=pd.solve(Sig[-j,-j]),dvec=rep(0,d-1),Amat=t(A),bvec=b)$solution
        Sig[j,-j] <- Sig[-j,j] <- y
      } else {
        ...
        y <- quadprog::solve.QP(Dmat=pd.solve(Sig[-j,-j]),dvec=rep(0,d-1),Amat=t(Ap),bvec=b,meq=length(eqs[[j]]))$solution
        Sig[j,-j] <- Sig[-j,j] <- y
      }
    }
    it <- it+1
