...
Here I run experiments stan to campare with those Laura runs in numpyro. I load the same data as her. We will also make timing comparisons
...
my_dir <- "/home/usuario/Documents/Barcelona_Yr1/GraphicalModels_NetworkData/LiLicode/Bayesian"
setwd(my_dir)
```
...
```{r Experiment_Settings_p10, include=TRUE,echo=TRUE, eval=TRUE,cache=TRUE}
...
```{r Data_Load_p10, include=TRUE,echo=TRUE, eval=TRUE,cache=TRUE}

data_folder <- "/GOLAZO_SS_SimData/data/"
#n_sims <- 10
#p <- 10

# Extract theta_true, mu_true, n_obs, As (the same for every file, using sim0)
data_path <- paste0(my_dir,data_folder,"sim0_p", p, "_n2000.json")
...
for (i in seq(0,N-1)){
  data_path <- paste0(my_dir,data_folder,"sim", i, "_p", p, "_n2000.json")
  data <- fromJSON(file=data_path)
...
# GOLAZO-Spike-and-Slab - A-semidep85 {.tabset}

## Thets vs A-semidep85

```{r Theta_vs_A-semidep85_p10, include=TRUE,echo=TRUE, eval=TRUE,cache=FALSE}

Theta_tri <- Theta[upper.tri(Theta)]
A_semidep85_tri <- A_semidep85[upper.tri(A_semidep85)]
par(mfrow = c(1, 2))
vioplot(Theta_tri[A_semidep85_tri <= 0], ylim = c(-1, 1), ylab = "Theta", names = c("A = 0"), main = paste("A-semidep85, p = ", p))
vioplot(Theta_tri[A_semidep85_tri > 0], ylim = c(-1, 1), ylab = "Theta", names = c("A = 1"))

```

## Prior Specification 

```{r GOLAZO_semidep85A_SS_prior_specification_p10, include=TRUE,echo=TRUE, eval=TRUE,cache=TRUE}

s0 <- 0.003
spike_scale_set <- 0.003

m_00_set_GOLAZO_SS_semidep85A <- 0
s_00_set_GOLAZO_SS_semidep85A <- 0.145
m_01_set_GOLAZO_SS_semidep85A <- -2.197
s_01_set_GOLAZO_SS_semidep85A <- 0.661
m_02_set_GOLAZO_SS_semidep85A <- -2.722
s_02_set_GOLAZO_SS_semidep85A <- 3.278
...
Network_SS_muFixed_stan <- stan_model(file = "Network_SS_muFixed.stan")

```

## Inference

```{r GOLAZO_semidep85A_SS_p10, include=TRUE,echo=TRUE, eval=TRUE,cache=TRUE}
...
for(j in 1:N){
  ...
  Learn_Beta_Bayesian_GOLAZO_semidep85A_partialCorrelation_SS_Indicator <- sampling(object = Network_SS_muFixed_stan, 
        ...
        , init = list(c1 = list(#mu = rep(0, p), 
           ...
           sqrt_theta_ii = rep(1, p),
                    #eta0 = c(m_00_set_GOLAZO_SS_semidep85A, 0),
                    #eta1 = c(m_01_set_GOLAZO_SS_semidep85A, 0),
                    #eta2 = c(m_02_set_GOLAZO_SS_semidep85A, 0),
                    tilde_eta0 = c(0, 0),
                    tilde_eta1 = c(0, 0),
                    tilde_eta2 = c(0, 0),
...
```{r GOLAZO_semidep85A_SS_results_p10, include=TRUE,echo=TRUE, eval=TRUE,cache=FALSE}
...
```{r GOLAZO_hyperparameters_p10, include=TRUE,echo=TRUE, eval=TRUE,cache=TRUE}
## Hyperparameters ##
...
```{r GOLAZO_semidep85A_p10, include=TRUE,echo=TRUE, eval=TRUE,cache=TRUE}
...
for(j in 1:N){
  ...
  ## Jack: min_coord is a matrix rather than a vector, because there are many minimums, all with the same objective
...
```{r GOLAZO_semidep85A_results_p10, include=TRUE,echo=TRUE, eval=TRUE,cache=FALSE}
...
```{r Experiment_Settings_p50, include=TRUE,echo=TRUE, eval=TRUE,cache=TRUE}

p <- 50 # dimension of observations/number of ocunties
N <- 10 # number of repeats
n <- 100 # numbe rof observations/time points
...
```{r Data_Load_p50, include=TRUE,echo=TRUE, eval=TRUE,cache=TRUE}
...
```{r Theta_vs_A-semidep85_p50, include=TRUE,echo=TRUE, eval=TRUE,cache=FALSE}
...
```{r GOLAZO_semidep85A_SS_prior_specification_p50, include=TRUE,echo=TRUE, eval=TRUE,cache=TRUE}

s0 <- 0.003
spike_scale_set <- 0.003

m_00_set_GOLAZO_SS_semidep85A <- 0
s_00_set_GOLAZO_SS_semidep85A <- 0.152
m_01_set_GOLAZO_SS_semidep85A <- -2.197
s_01_set_GOLAZO_SS_semidep85A <- 0.661
m_02_set_GOLAZO_SS_semidep85A <- -6.737
s_02_set_GOLAZO_SS_semidep85A <- 3.395
...
```{r GOLAZO_semidep85A_SS_p50, include=TRUE,echo=TRUE, eval=TRUE,cache=TRUE}
...
```{r GOLAZO_semidep85A_SS_results_p50, include=TRUE,echo=TRUE, eval=TRUE,cache=FALSE}
...
```{r GOLAZO_hyperparameters_p50, include=TRUE,echo=TRUE, eval=TRUE,cache=TRUE}
## Hyperparameters ##
ebic.gamma <- 0    # when equals to 0, it's BIC; when equals to 0.5, it's EBIC
edge.tol   <- 0.01   
...
```{r GOLAZO_semidep85A_p50, include=TRUE,echo=TRUE, eval=TRUE,cache=TRUE}
...
```{r GOLAZO_semidep85A_results_p50, include=TRUE,echo=TRUE, eval=TRUE,cache=FALSE}
