...
my_res = model_mcmc_run(Y=covid_vals, my_model=my_model, my_model_args=my_model_args, n_warmup=n_warmup, 
...
with open(data_save_path + f'NetworkSS_geo_sci_1mcmc.sav' , 'wb') as f:
...
my_res = model_mcmc_run(Y=covid_vals, my_model=my_model, my_model_args=my_model_args, 
                   n_warmup=n_warmup, n_samples=n_samples,fix_params=fix_params, fixed_params_dict=fixed_params_dict, 
                   blocked_params_list=blocked_params_list,  estimates_print=estimates_print,
                   my_init_strategy=my_init_strategy,  
                   verbose=verbose, key_no_run=p)

with open(data_save_path + f'NetworkSS_geo_sci_2mcmc.sav' , 'wb') as f:
    pickle.dump((my_res), f)
