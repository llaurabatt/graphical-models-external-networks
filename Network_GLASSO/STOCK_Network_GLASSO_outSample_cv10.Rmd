---
title: "Stock Network GLASSO out-of-sample Analysis - 10-fold cross-validation"
author: "Jack Jewson"
date: "19/07/2022"
...
set.seed(1234)
stock_seq_shuffle <- sample(seq_len(nrow(stock_trans)), nrow(stock_trans), replace = FALSE)

for (i in 1:(N_folds - 1)){
  picked <- (i - 1)*n + 1:n 
  train_data[[i]] <- stock_trans[-stock_seq_shuffle[picked], ]
  test_data[[i]]  <- stock_trans[stock_seq_shuffle[picked], ]
}
not_picked <- 1:((N_folds - 1)*n) 
train_data[[N_folds]] <- stock_trans[stock_seq_shuffle[not_picked], ]
test_data[[N_folds]]  <- stock_trans[-stock_seq_shuffle[not_picked], ]
...
beta0_grid_min <- -3
beta0_grid_max <- rep(-0.5, N)
beta0_grid_min <- rep(-2, N)
...
beta0_grid_length <- 20
beta0_grid_max <- c(-0.5, 0, -0.5, -0.5, -0.5, -0.5, -0.5, 0, -0.5, -0.5)
beta0_grid_min <- beta0_grid_max - 1.5
...
beta1_grid_length <- 20
beta1_grid_max <- c(0.5, 0, 0.5, 0.5, 0.5, 0.5, 0.5, 0, 0.5, 0.5)
beta1_grid_min <- beta1_grid_max - 1.5
...
beta0_grid_length <- 20
beta0_grid_max <- rep(0, N)
beta0_grid_min <- rep(-1.5, N)
...
beta2_grid_length <- 20
beta2_grid_max <- rep(0, N)
beta2_grid_min <- rep(-1.5, N)
...
beta0_grid_length <- 10
beta0_grid_max <- c(-0.25, -0.75, -0.75, -0.75, -0.75, -0.25, -0.75, -0.75, -0.75, -0.75)
beta0_grid_min <- beta0_grid_max - 1
...
beta1_grid_max <- c(0.5, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25)
beta1_grid_min <- beta1_grid_max - 1
...
beta2_grid_max <- c(0, 0.5, 0.5, 0.5, 0.5, 0.25, 0.5, 0.5, 0.5, 0.5)
beta2_grid_min <- beta2_grid_max - 1
...
mean(GOLAZO_A1_sim_results[,22])*round(nrow(stock_trans)/N_folds)
mean(GOLAZO_A2_sim_results[,22])*round(nrow(stock_trans)/N_folds)
mean(GOLAZO_A1A2_sim_results[,32])*round(nrow(stock_trans)/N_folds)
